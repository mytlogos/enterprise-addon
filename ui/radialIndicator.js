/*
    ...
    Repository: https://github.com/s-yadav/radialIndicator
*/

/**
 * Modified to remove eval.
 * Eval violates the CSP in the popup environment of the extension.
 */
(function (factory) {
    /** support UMD ***/
    let global = window;
    ...
    let smoothCanvas = (function () {
        ...
        return function (w, h, canvasElm) {
            let can = canvasElm || document.createElement("canvas");
            can.width = w * ratio;
            can.height = h * ratio;
    ...
    function getCurrentColor(curPer, bottomVal, topVal, bottomColor, topColor) {
        let rgbAryTop = topColor.indexOf('#') !== -1 ? hexToRgb(topColor) : topColor.match(/\d+/g),
    ...
    function Indicator(container, indOption) {
        ...
        let canElm = document.createElement("canvas");
        ...
        let startListener = function (e) {
            ...
            let touchMove = e.type === "touchstart" ? "touchmove" : "mousemove",
            ...
            let moveListener = function (e) {
                ...
                //get the cordinates
                let mx = e.clientX || e.touches[0].clientX,
    ...
    Indicator.prototype = {
        ...
        value: function (val) {
            ...
            let precision = indOption.precision != null ? indOption.precision : 0,
                precisionNo = Math.pow(10, precision),
            ...
            if (typeof curColor === "object") {
                let range = Object.keys(curColor);

                for (let i = 1, ln = range.length; i < ln; i++) {
                    let bottomVal = range[i - 1],
                        topVal = range[i],
    ...
    function radialIndicator(container, options) {
        let progObj = new Indicator(container, options);
        progObj._init();
        return progObj;
    }
    ...
    if ($) {
        $.fn.radialIndicator = function (options) {
            return this.each(function () {
                let newPCObj = radialIndicator(this, options);
